...
该接口提交一个 PPT 视频切图任务。算子将图片直接写入平台共享结果目录，全部完成后原子发布 `manifest.json`，最后仅发送一次终态元数据回调。协议不包含 Base64 图片，也不逐图回调。
  ...
  "images": [
    {
      "frame_seq": 217,
      "snap_time": 216,
      "path": "/data/result/course-001/ppt/slices/ppt-0001-217-216.jpg"
    }
  ]
}
```
