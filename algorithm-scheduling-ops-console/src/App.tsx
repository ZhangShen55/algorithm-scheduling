...
import { aggregateGpuMetrics, consoleConfigSource, defaultConsoleConfig, demoActiveLeases, demoData, demoTask, demoTaskList, deploymentTemplateConfig, emptyConsoleData, fetchActiveLeases, fetchConsoleData, fetchGpuNodeStates, fetchTask, fetchTaskList, loadConsoleConfig, mergeConsoleGpuSnapshots, mergeGpuNodeStates, saveConsoleConfig, testControlConnection, testGatewayConnection, testGpuConnection, validateGpuNodeConfigs } from './api'
import { appendTrend, initialTrend } from './trend'
import { KafkaEvents } from './KafkaEvents'
import { TaskWorkspace } from './TaskWorkspace'
import type { ActiveLeaseResponse, CapacitySnapshot, ConnectionTestResult, ConsoleConfig, ConsoleData, GpuMetrics, OperationsTrendPoint, OperatorInstance, TaskDetail, TaskListResponse, VisualStyle } from './types'
...
function App() {
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      ...
      setData((current) => mergeConsoleGpuSnapshots(current, next))
      setError('')
    } catch (reason) {
      if (version !== refreshVersion.current) return
  ...
  const refreshGpu = useCallback(async () => {
    ...
    try {
      const nextGpuNodes = await fetchGpuNodeStates(consoleConfig)
      if (version !== gpuRequestVersion.current) return
      setData((current) => { const gpuNodes = mergeGpuNodeStates(current.gpuNodes, nextGpuNodes); return { ...current, gpu: aggregateGpuMetrics(gpuNodes), gpuNodes } })
    } catch {
      if (version !== gpuRequestVersion.current) return
      setData((current) => ({ ...current, gpu: { status: 'unavailable', sampled_at: Date.now() / 1000, devices: [], error: 'GPU 指标接口暂不可用' }, gpuNodes: [] }))
    } finally { gpuInFlight.current = false }
  }, [consoleConfig])
  ...
  return <div className={`shell ${sidebarCollapsed ? 'sidebar-collapsed' : ''}`} data-style={visualStyle}>
    ...
    <main className="main">
      ...
      <div className="content">
        ...
        {view === 'instances' && <Instances instances={data.instances} snapshots={data.snapshots} gpu={data.gpu} gpuNodes={data.gpuNodes} filter={instanceFilter} setFilter={setInstanceFilter} onSelect={setSelectedInstance} />}
        {view === 'tasks' && <TaskWorkspace config={consoleConfig} source={data.source} autoRefresh={autoRefresh} />}
        {view === 'gateway' && <Gateway gateway={data.gateway} trendPoints={trendPoints} visualStyle={visualStyle} source={data.source} />}
        {view === 'system' && <System data={data} config={consoleConfig} />}
      </div>
    </main>
...
function ConsoleConfigPanel({ value, onSave, onReset, onClose }: { value: ConsoleConfig; onSave: (value: ConsoleConfig) => void; onReset: () => void; onClose: () => void }) {
  ...
  const template = deploymentTemplateConfig()
  const gpuNodes = draft.gpuNodes || []
  const sourceText = { browser: '浏览器保存', build: '构建配置', 'deployment-template': '部署地址模板' }[consoleConfigSource()]
  ...
  async function testConnection() {
    setTesting(true)
    setTestResults([])
    setTestResults(await Promise.all([testControlConnection(draft), testGatewayConnection(draft), testGpuConnection(draft)]))
    setTesting(false)
  }
  ...
  return <div className="drawer-backdrop" onClick={onClose}><aside className="drawer config-drawer" onClick={event => event.stopPropagation()}><div className="drawer-head"><div><h2>连接与观测配置</h2><span className="config-source">当前来源：{sourceText}</span></div><button className="icon-button" onClick={onClose} aria-label="关闭配置"><X size={18} /></button></div><div className="config-intro">默认模板使用当前页面 IP，配置仅保存在本浏览器。</div><form className="config-form" onSubmit={(event) => { event.preventDefault(); const validationError = validateGpuNodeConfigs(gpuNodes); if (validationError) { setTestResults([{ service: 'gpu', ok: false, message: validationError }]); return } onSave(draft) }}><label><span>Control Service 地址</span><input value={draft.controlBaseUrl} onChange={(event) => update('controlBaseUrl', event.target.value)} placeholder={template.controlBaseUrl} required /><small>模板：{template.controlBaseUrl}</small></label><label><span>gateway-online 地址</span><input value={draft.gatewayBaseUrl} onChange={(event) => update('gatewayBaseUrl', event.target.value)} placeholder={template.gatewayBaseUrl} required /><small>模板：{template.gatewayBaseUrl}</small></label><div className="gpu-node-config"><div className="config-section-head"><div><strong>GPU 指标主机</strong><small>每台主机运行一个只读 GPU Exporter</small></div><button type="button" className="secondary-button" onClick={() => setDraft(current => ({ ...current, gpuNodes: [...(current.gpuNodes || []), { hostId: '', name: `GPU 主机 ${(current.gpuNodes || []).length + 1}`, url: 'http://127.0.0.1:9400', enabled: true }] }))}>添加主机</button></div>{gpuNodes.map((node, index) => <div className="gpu-node-row" key={`${node.hostId}-${index}`}><label><span>服务器 IP</span><input value={node.hostId} onChange={event => setDraft(current => ({ ...current, gpuNodes: (current.gpuNodes || []).map((item, itemIndex) => itemIndex === index ? { ...item, hostId: event.target.value } : item) }))} placeholder="192.168.29.11" required /></label><label><span>显示名称</span><input value={node.name} onChange={event => setDraft(current => ({ ...current, gpuNodes: (current.gpuNodes || []).map((item, itemIndex) => itemIndex === index ? { ...item, name: event.target.value } : item) }))} placeholder="平台 GPU 主机" /></label><label><span>Exporter 地址</span><input value={node.url} onChange={event => setDraft(current => ({ ...current, gpuNodes: (current.gpuNodes || []).map((item, itemIndex) => itemIndex === index ? { ...item, url: event.target.value } : item) }))} placeholder="http://192.168.29.11:9400" required /></label><label className="gpu-node-enabled"><span>启用</span><input type="checkbox" checked={node.enabled} onChange={event => setDraft(current => ({ ...current, gpuNodes: (current.gpuNodes || []).map((item, itemIndex) => itemIndex === index ? { ...item, enabled: event.target.checked } : item) }))} /></label>{gpuNodes.length > 1 && <button type="button" className="icon-button" onClick={() => setDraft(current => ({ ...current, gpuNodes: (current.gpuNodes || []).filter((_, itemIndex) => itemIndex !== index) }))} aria-label={`删除 GPU 主机 ${index + 1}`} title="删除 GPU 主机"><X size={15} /></button>}</div>)}</div><div className="config-grid"><label><span>总览刷新周期（秒）</span><input type="number" min="1" max="60" step="1" value={draft.refreshSeconds} onChange={(event) => update('refreshSeconds', event.target.value)} /></label><label><span>实例任务刷新周期（秒）</span><input type="number" min="1" max="30" step="1" value={draft.leaseRefreshSeconds} onChange={(event) => update('leaseRefreshSeconds', event.target.value)} /></label><label><span>GPU 刷新周期（秒）</span><input type="number" min="1" max="30" step="1" value={draft.gpuRefreshSeconds} onChange={(event) => update('gpuRefreshSeconds', event.target.value)} /></label></div><div className="connection-results">{testResults.map((result) => <div key={result.service} className={`config-result ${result.ok ? 'success' : 'failure'}`}><span className="health-dot" /><strong>{result.service === 'control' ? 'Control' : result.service === 'gateway' ? 'Gateway' : 'GPU'}</strong><span>{result.message}</span></div>)}</div><div className="config-actions"><button type="button" className="secondary-button" onClick={() => { setDraft(template); setTestResults([]); onReset() }}>恢复默认地址</button><button type="button" className="secondary-button" onClick={() => void testConnection()} disabled={testing}>{testing ? '分别读取中' : '测试三个服务'}</button><button type="submit" className="primary-button">保存并应用</button></div></form></aside></div>
}
...
function Overview({ data, totals, trendPoints, visualStyle, onInstance, onView }: { data: ConsoleData; totals: { online: number; ready: number; capacity: number; used: number; waiting: number }; trendPoints: OperationsTrendPoint[]; visualStyle: VisualStyle; onInstance: (instance: OperatorInstance) => void; onView: (view: View) => void }) {
  ...
  return <>
    ...
    <section className="panel gpu-overview-panel"><PanelTitle title="整机 GPU 状态" action="查看实例清单" onAction={() => onView('instances')} /><GpuCards gpu={data.gpu} gpuNodes={data.gpuNodes} instances={data.instances} /></section>
  </>
}
...
export function Instances({ instances, snapshots, gpu, gpuNodes, filter, setFilter, onSelect }: { instances: OperatorInstance[]; snapshots: CapacitySnapshot[]; gpu: GpuMetrics; gpuNodes: ConsoleData['gpuNodes']; filter: string; setFilter: (value: string) => void; onSelect: (item: OperatorInstance) => void }) {
  ...
  const hosts = [...new Set([...gpuNodes.map((item) => item.config.hostId), ...instances.map((item) => hostIdFor(item)).filter((value): value is string => Boolean(value))])]
  const instanceHost = (instance: OperatorInstance) => hostIdFor(instance) || (gpuNodes.length === 1 && gpuIndexFor(instance) !== undefined ? gpuNodes[0].config.hostId : undefined)
  const visible = instances.filter((instance) => {
    ...
    return (filter === 'all' || instance.operator_code === filter)
      ...
      && (host === 'all' || instanceHost(instance) === host)
  ...
  const visibleGpuNodes = host === 'all' ? gpuNodes : gpuNodes.filter((node) => node.config.hostId === host)
  const hostInstances = host === 'all' ? instances : instances.filter((instance) => instanceHost(instance) === host)
  return <section className="instances-view">
    <section className="panel gpu-instance-panel">
      <PanelTitle title="整机 GPU 与算子部署" />
      <GpuCards gpu={gpu} gpuNodes={visibleGpuNodes} instances={hostInstances} />
    </section>
    <div className="panel full-panel">
      <div className="table-toolbar instance-toolbar">
        <div><h2>实例清单</h2><span className="panel-subtitle">容量、运行状态和 GPU 部署集中查看</span></div>
        <div className="instance-filters">
          <label>算子<select value={filter} onChange={event => setFilter(event.target.value)}><option value="all">全部算子</option>{Object.entries(operatorNames).map(([code, name]) => <option value={code} key={code}>{name}</option>)}</select></label>
          <label>服务器<select value={host} onChange={event => setHost(event.target.value)}><option value="all">全部服务器</option>{hosts.map(item => <option value={item} key={item}>{item}</option>)}</select></label>
          <label>生命周期<select value={lifecycle} onChange={event => setLifecycle(event.target.value)}><option value="all">全部状态</option><option value="ONLINE">在线</option><option value="DRAINING">排空中</option><option value="OFFLINE">离线</option></select></label>
          <label>设备<select value={device} onChange={event => setDevice(event.target.value)}><option value="all">全部设备</option>{devices.map(item => <option value={item} key={item}>{item}</option>)}</select></label>
          <label>模型<select value={modelReady} onChange={event => setModelReady(event.target.value)}><option value="all">全部模型</option><option value="true">已就绪</option><option value="false">未就绪</option></select></label>
          <label>活动任务<select value={activity} onChange={event => setActivity(event.target.value)}><option value="all">全部任务</option><option value="active">有活动任务</option><option value="idle">无活动任务</option></select></label>
        </div>
      </div>
      <div className="table-wrap"><table><thead><tr><th>实例</th><th>算子</th><th>服务器</th><th>GPU</th><th>模型</th><th>容量使用</th><th>有效租约</th><th>心跳</th><th>状态</th><th /></tr></thead><tbody>
        {sorted.map(instance => {
          const snapshot = snapshotById.get(instance.instance_id)
          const percent = Math.round((snapshot?.schedulable_used ?? instance.inflight) / Math.max(1, instance.declared_capacity) * 100)
          const topology = topologyDisplayFor(instance, gpuNodes)
          return <tr key={instance.instance_id} onClick={() => onSelect(instance)}>
            <td><div className="instance-cell"><span className={`health-dot ${instance.lifecycle.toLowerCase()}`} /><strong>{instance.instance_id}</strong></div></td>
            <td><span className="code-label">{operatorNames[instance.operator_code] || instance.operator_code}</span></td>
            <td><span className="device-label">{topology.host}</span></td>
            <td><span className="device-label">{topology.gpu}</span></td>
            <td><span className={instance.model_ready ? 'model-ready' : 'model-not-ready'}>{instance.model_ready ? 'READY' : 'NOT READY'}</span><small>{instance.model_version || '未上报'}</small></td>
            <td><div className="table-capacity"><span>{snapshot?.schedulable_used ?? instance.inflight} / {instance.declared_capacity}</span><i><b style={{ width: `${Math.min(100, percent)}%` }} /></i></div></td>
            <td>{snapshot?.active_lease_count ?? '-'}</td>
            <td><span className="heartbeat">{relativeTime(instance.last_heartbeat_at)}</span></td>
            <td><span className={`pill ${instance.lifecycle.toLowerCase()}`}>{instance.lifecycle === 'ONLINE' ? '在线' : instance.lifecycle === 'DRAINING' ? '排空中' : '离线'}</span></td>
            <td><ChevronRight size={16} className="row-arrow" /></td>
          </tr>
        })}
      </tbody></table></div>
      <div className="table-footer">显示 {sorted.length} / {instances.length} 个实例 <span>点击实例查看当前 task_id · work_type</span></div>
    </div>
  </section>
}

function GpuCards({ gpu, gpuNodes, instances }: { gpu: GpuMetrics; gpuNodes: ConsoleData['gpuNodes']; instances: OperatorInstance[] }) { const nodes = gpuNodes.length ? gpuNodes : [{ config: { hostId: gpu.host_id || '未标记主机', name: 'GPU 主机', url: '', enabled: true }, status: gpu.status, metrics: gpu, error: gpu.error }]; return <div className="gpu-host-list">{nodes.map((node) => <section className="gpu-host-group" key={node.config.hostId}><div className="gpu-host-heading"><div><strong>{node.config.name}</strong><span>{node.config.hostId}{node.lastSuccessAt ? ` · 最近成功 ${formatClock(node.lastSuccessAt)}` : ''}</span></div><span className={`source-chip ${node.status === 'ok' ? 'live' : 'error'}`}>{node.status === 'ok' ? '在线' : node.status === 'identity_error' ? '身份异常' : '读取失败'}{node.compatibility ? ' · 兼容映射' : ''}</span></div>{node.status !== 'ok' && <div className="gpu-node-error"><AlertTriangle size={14} />{node.error || 'GPU 指标不可用'}{node.metrics && node.status !== 'identity_error' ? ' · 显示最近成功数据' : ''}</div>}{!node.metrics ? <div className="gpu-unavailable"><AlertTriangle size={15} />暂无可用 GPU 快照</div> : <div className="gpu-card-grid">{node.metrics.devices.map((device) => { const deployed = node.status === 'identity_error' ? [] : instances.filter((instance) => (hostIdFor(instance) === node.config.hostId || (node.compatibility && !hostIdFor(instance))) && gpuIndexFor(instance) === device.index); return <div className="gpu-card" key={`${node.config.hostId}-${device.index}`}><div className="gpu-card-head"><span className="gpu-index">GPU {device.index}</span><strong>{device.name}</strong><span className={`gpu-load ${device.utilization_percent >= 85 ? 'high' : device.utilization_percent >= 60 ? 'medium' : ''}`}>{device.utilization_percent}%</span></div><div className="gpu-bar"><i style={{ width: `${Math.min(100, device.utilization_percent)}%` }} /></div><div className="gpu-stats"><span><b>{formatBytes(device.memory_used_bytes)}</b><small>显存 / {formatBytes(device.memory_total_bytes)}</small></span><span><b>{device.temperature_celsius ?? '-'}°C</b><small>温度</small></span><span><b>{device.power_watts ?? '-'} W</b><small>功耗</small></span></div><div className="gpu-deployed"><span>已部署算子</span><div>{deployed.length ? deployed.map((instance) => <span className="gpu-operator" key={instance.instance_id} title={instance.instance_id}>{instance.operator_code}</span>) : <small>未部署算子</small>}</div></div></div> })}</div>}</section>)}</div> }
...
export function hostIdFor(instance: OperatorInstance): string | undefined { const value = instance.labels.host_id?.trim(); return value || undefined }
export function gpuIndexFor(instance: OperatorInstance): number | undefined { const value = instance.labels.gpu || instance.labels.gpu_index; const parsed = value === undefined ? NaN : Number(value); return Number.isInteger(parsed) && parsed >= 0 ? parsed : undefined }
export function topologyDisplayFor(instance: OperatorInstance, gpuNodes: ConsoleData['gpuNodes']): { host: string; gpu: string } {
  const hostId = hostIdFor(instance)
  const gpuIndex = gpuIndexFor(instance)
  const rawGpu = instance.labels.gpu ?? instance.labels.gpu_index
  const host = hostId || (gpuNodes.length === 1 && gpuIndex !== undefined
    ? `${gpuNodes[0].config.hostId} · 兼容映射`
    : '未标记主机')
  const gpu = rawGpu === undefined || rawGpu.trim() === ''
    ? '未标记 GPU'
    : gpuIndex === undefined ? 'GPU 标签无效' : `GPU ${gpuIndex}`
  return { host, gpu }
}
