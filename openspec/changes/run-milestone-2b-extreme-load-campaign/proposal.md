...
- 为 PPT OCR 的幂等单图 HTTP 调用增加配置化、有界的瞬时网络异常重试，并确保最终失败状态保留异常类型和非空中文原因；不重试 OCR 业务错误或任意未知异常。
- 为 PPT Slice 的幂等异步提交增加配置化、有界的瞬时网络异常重试；相同 `operator_task_id` 和相同载荷的在途重复提交由算子返回既有受理状态，冲突载荷继续拒绝，超时、HTTP、业务和未知错误不重试。
