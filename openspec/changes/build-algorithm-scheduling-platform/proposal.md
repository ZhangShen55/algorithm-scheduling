...
>
> 本文中的 Text Analysis、PPT 关键词和课程脑图属于当时已经实现的历史平台范围，原文与完成
> 事实继续保留。当前平台已通过 `retire-text-analysis-from-scheduling-platform` 收敛为七算子，
> 新 PPT DAG 只到 OCR，新 ASR DAG 只到转写；后续不得再以本文恢复退役节点或部署项。
