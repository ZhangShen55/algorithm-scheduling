...
- [x] 9.8 补充 Harness 场景、验证索引、变更账本和远端受控证据，使用中文 Conventional Commit 提交推送，最终运行 Harness consistency、相关平台回归、OpenSpec strict 和 `git diff --check`。收口提交 `25ef7e2aaa5e3da24073dc9cb158c7e556a9a064`（`docs(harness): 收口 FaceRec GPU 进程命名证据`）已推送；本地聚焦回归 `86 passed, 1 skipped`，目标 Linux 入口测试 `10 passed`，OpenSpec strict 与 `git diff --check` 通过。远端 8 个证据文件均为 `0600`、单硬链接，manifest 和脱敏扫描通过，证据绑定镜像 revision `cc3c78dbcd608f77461d7d39bc5c5c6d156d6276`。
