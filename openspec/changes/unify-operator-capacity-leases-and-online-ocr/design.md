...
Vision Orchestrator 先在 PostgreSQL 中持久化节点结果和终态，再发布视觉完成事件。Orchestrator 消费视觉事件时必须先校验 `task_id/task_type/node_id` 与任务事实一致。如果进度事件初读节点为 `RUNNING`，但在进度写入取得数据库行锁前节点已被另一事务提交为 `COMPLETED`，消费端应重读节点并将该迟到进度幂等提交，不得覆盖终态或终止 Consumer。重复完成事件不重复持久化结果，已完成的任务类型不再重复汇总。节点仍非终态时的仓储校验错误、任务身份不一致或终态早于持久化仍必须失败关闭且不提交 Kafka offset。
