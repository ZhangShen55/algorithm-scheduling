...
- [ ] 14.3 运行里程碑 2B 的 PPT、ASR、教师/学生视觉及在线链路回归，确认新增 OCR 不破坏当前 `ppt-ocr-关键字` 泳道和既有 A 服务接口；教师/学生长视频粗扫还必须证明 ffmpeg/ffprobe 进程受 `media.max_concurrent_processes` 约束，Vision Orchestrator 的 `4G` cgroup 无 OOM 且 `/ready` 保持就绪；平台先于 VBas 启动时，已取得命令必须不提交 offset 地等待容量，Consumer 和 `/ready` 不得因此失败。
