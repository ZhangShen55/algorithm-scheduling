...
>
> 本文对 Text Analysis 容量、关键词/课程脑图租约及八算子部署的描述保留为已实现历史事实。
> 当前最终验收已由 `retire-text-analysis-from-scheduling-platform` 调整为七算子、21 实例、
> PPT/OCR 和 ASR-only；不得用本文恢复退役算子或节点。
