...
`media.batch_extraction_enabled=true` 时，同一 batch 内的等间隔时间点由一次 ffmpeg 连续解码
生成，避免逐帧重复启动进程和随机 seek；不同课程和不同 batch 仍可并行使用全部
`media.max_concurrent_processes` 槽位。批量命令无法完整生成目标帧时自动回退到原有并行单帧
抽取，保证异常编码和可变帧率视频仍可处理。

教师泳道先按可配置粗粒度扫描，命中板书或坐姿后按
`scan.refinement_intervals_seconds` 逐级加密；每层检测后都会依据本层阳性点重新收敛候选
窗口，下一层只扫描收敛后的范围。教师行为区间和学生人数结果写入现有
...
计算前排和后排人数；旧实例没有返回完整坐标时才回退为区域推理。节点查询中的整数状态码
保持不变，`reason` 使用同一条流水线进度同时报告抽帧和推理 batch 数，避免并行协程互相覆盖
后留下已经过期的“等待 VBas”状态。
