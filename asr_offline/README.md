...
2. 确保项目根目录存在 `model/`，镜像会把运行所需模型文件直接打入镜像，不需要运行时挂载模型目录。
3. 基础镜像为 **CentOS 7**（`glibc 2.17`），Dockerfile 已固定 `Miniconda3-py310_23.11.0-2`，勿改用 `Miniconda3-latest`（会报 `GLIBC >=2.28`）。
4. 构建上下文已通过 `.dockerignore` 排除 `tests/`、`test_wav/`、日志等非运行文件，但不会排除 `model/` 内的权重文件。
...
`obfuscator` 阶段会将 `main.py`、`core/`、`utils/`、`api/`、`entity/` 生成为 PyArmor 混淆产物；运行层只复制混淆后的业务代码。
