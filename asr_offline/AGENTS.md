...
This service transcribes completed course audio and returns offline ASR results for persistence by the scheduling workflow. Preserve the `/v1.1.8/seacraft_asr` contract. The retired `/v1.1.7/seacraft_asr`, `/audio/detect_mandarin`, and `/text/question` routes must not be restored.
...
Route verification must confirm that `/v1.1.8/seacraft_asr` and `/audio/db_snr` remain available, while `/v1.1.7/seacraft_asr`, `/audio/detect_mandarin`, and `/text/question` return 404 and do not appear in OpenAPI.
