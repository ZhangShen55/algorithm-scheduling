...
## 2026-09-03 - 20 路全量与在线 200 并发混合压力及媒体清理验证

- Run ID `cleanup-mixed-20full-200x10000-20260903-174427` 在 `192.168.29.11` 同时提交
  20 路全量课程任务和在线人数识别 200 并发、10000 次请求，总墙钟约 70 分 5 秒。
- 20/20 提交和课程终态完成；PPT、ASR、学生行为全部成功，教师行为 18 成功、2 失败。
  两项失败与 Control 容器在 18:06:32 被其他发布操作替换的窗口重合，暴露 Vision 对容量租约
  瞬时连接错误缺少有限重试的恢复缺陷。100 个节点均只领取 1 次，无状态冲突复现。
- 在线请求 HTTP 200 为 10000/10000，但业务成功 9884、失败 116，成功率 98.84%；现有日志
  无法可靠区分容量等待超时、租约连接故障和下游连接故障，在线门禁判定未通过。
- 三台 VBas 均处理了在线和离线真实请求，显存从本轮约 13 GiB 基线短时升至
  14.2–14.4 GiB 后回落；最终租约、reported inflight、Outbox 和三个 Kafka Consumer lag 均归零。
