...
新租约申请继续要求实例心跳有效；既有且尚未过期的在途租约由调用方按时续期时，不得仅因
算子单 Worker 推理短暂阻塞心跳而失效。验证必须同时证明心跳缺失期间没有新租约进入、原
租约仍占用容量，且注销、重注册、显式离线和租约自身到期仍能终止旧租约。
...
### 2026-08-22 Attempt 13 复核事实更正

- 上一节关于 `5f973ada...` 的“5项受控复核产物与 Git 外安全索引已经校验”不符合服务器最终
  事实：当前 release 只存在 offline review request，没有 review input、index、artifact 或
  publisher 调用。本条只追加更正，不删除原历史文本。
- 独立复核实际判定 `PPT-014` 漏切1张稳定标注页、`ASR-013` 在24个双语术语片段中有9个严重
  错误；其余3项 offline 复核通过。由于复核集合必须整体发布，5项均没有形成通过 artifact。
- PPT 阈值 `0.99` 的完整视频探针已经解除已观察到的漏切且没有引入动态区间爆发切片；ASR 热词
  探针没有改善9/24结果，受控配置仍禁用热词。该 SHA 继续只作为失败诊断和精确恢复证据，
  不计入本变更或容量变更的最终通过。
