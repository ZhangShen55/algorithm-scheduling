...
- 极限 Campaign 的 Kafka lag 使用独立 `20s/2 次/0.25s` 探针合同；其他只读采集面默认使用单次 5 秒超时、同采样最多 2 次、间隔 0.25 秒。全部尝试失败必须发布脱敏失败证据并永久锁存 STOP，后续恢复不得覆盖。故障语义探针必须通过 `deploy/scripts/extreme-load-fault-probe` 使用平台 `.venv`，不得依赖目标机全局 Python 包。
