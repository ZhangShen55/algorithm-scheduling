...
def test_compose_documents_stable_host_ports() -> None:
    ...
    assert "${PLATFORM_BIND_HOST:-0.0.0.0}:5432:5432" in document["services"]["postgres"]["ports"]
    assert "${PLATFORM_BIND_HOST:-0.0.0.0}:9092:9092" in document["services"]["kafka"]["ports"]
    assert "${PLATFORM_BIND_HOST:-0.0.0.0}:6379:6379" in document["services"]["redis"]["ports"]
    assert "127.0.0.1:27017:27017" in document["services"]["mongodb"]["ports"]

    instructions = DEPLOYMENT_DOC.read_text(encoding="utf-8")
    assert "PLATFORM_BIND_HOST" in instructions
    assert "PLATFORM_ADVERTISED_HOST" in instructions
...
def test_kafka_advertises_distinct_host_and_container_listeners() -> None:
    ...
    assert "EXTERNAL://${PLATFORM_ADVERTISED_HOST:-192.168.29.11}:9092" in environment["KAFKA_ADVERTISED_LISTENERS"]
    ...
    assert "EXTERNAL://127.0.0.1:9092" not in environment["KAFKA_ADVERTISED_LISTENERS"]
    assert "INTERNAL://127.0.0.1:29092" not in environment["KAFKA_ADVERTISED_LISTENERS"]
