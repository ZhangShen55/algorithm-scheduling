...
PLATFORM_ROOT = SERVICE_ROOT.parents[1]
...
def test_canonical_and_compatibility_entrypoints_share_routes() -> None:
    from services.online_gateway_service.app.main import app as canonical_app
    from services.online_gateway_service.main import app as compatibility_app

    assert compatibility_app is canonical_app
    assert canonical_app.title == "online-gateway-service"
    assert {route.path for route in canonical_app.routes} >= {
        "/health",
        "/ready",
        "/api/online/vbas/analyze",
        "/api/online/face/recognize",
        "/api/online/image-quality/detect",
        "/api/online/asr/stream",
    }


def test_default_lifespan_does_not_require_storage_directories() -> None:
    from services.online_gateway_service.app.main import app
...
def test_app_main_imports_from_service_working_directory() -> None:
    ...
    environment["PYTHONPATH"] = str(PLATFORM_ROOT)
...
def test_settings_precedence_is_defaults_then_toml_then_environment(
    ...
    from services.online_gateway_service.app.core.config import OnlineGatewaySettings
