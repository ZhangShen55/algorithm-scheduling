...
def test_canonical_and_compatibility_entrypoints_share_the_app() -> None:
    from services.vision_orchestrator_service.app.main import app as canonical_app
    from services.vision_orchestrator_service.main import app as compatibility_app

    assert compatibility_app is canonical_app
    assert canonical_app.title == "vision-orchestrator-service"
    assert {route.path for route in canonical_app.routes} >= {"/health", "/ready"}
...
def test_settings_precedence_is_defaults_then_toml_then_environment(
    ...
    from services.vision_orchestrator_service.app.core.config import VisionSettings
