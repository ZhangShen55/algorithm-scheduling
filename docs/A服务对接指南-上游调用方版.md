...
| `50301` | 在线等待算子容量超时，或 Control 服务不可用 | 网关已按配置等待；A 可在总预算内重新提交并保留追踪标识 |
...
一个等待稍后执行的在线请求；网关会在实例容量释放或轮询重试成功后继续处理，默认最长等待 300 秒。
...
决定；不支持的语言或未就绪的小语种模型会在异步节点中失败。已完成 ASR 在相同参数下复用；
后续使用不同 `asr_options` 时保留历史版本并创建新的 `run_id` 重新处理。
...
- 教师路由调用 `/ImageDetect/teacher/v1.0.0`；学生路由为 `POST /online/vbas/student`，调用
  `/ImageDetect/student/v1.0.0`；纯人数路由为 `POST /online/vbas/person-count`，调用
  `/AE/SyncTasks2`。三个路由均保持对应 VBas 请求和响应结构，不再使用 `stream_type`。
...
| HTTP 200、外层 code=50301 | 是，有界 | 网关已等待最多 300 秒；超时后 A 按总预算指数退避并告警 |
...
| 在线 VBas 教师 | POST | /online/vbas/teacher | Online Gateway |
| 在线 VBas 学生 | POST | /online/vbas/student | Online Gateway |
| 在线 VBas 人数 | POST | /online/vbas/person-count | Online Gateway |
...
涉及容量耗尽、算子异常或网络故障注入的用例，由平台方配合 A 完成。
...
- 平台/运维负责人向 A 交付多机与共享存储验收结论；A 只验收北向接口和业务结果。
...
TLS、身份认证、授权、限流和审计。A 只连接平台交付的 Control Service 和 Online Gateway 北向
入口，不使用平台内部服务地址或算子端口。
...
媒体域名/网段做业务白名单校验，不能只依赖 DNS 名称字符串，并应考虑 DNS 解析结果及重绑定风险。
平台侧的出站访问限制由平台团队负责。
...
本文复制版于 2026-08-27 按当前工作区实现核验。部署可覆盖容量、大小、超时和存储参数；每次
发布前应重新执行第 10 节联调清单，并把实际环境差异通过受控配置或发布说明交付，不在文档中写入
服务器密码、私钥、Token 或其他凭据。
