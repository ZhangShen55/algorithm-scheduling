...
### 25.1 当前实现成熟度（2026-08-06）

| 范围 | 当前证据 | 结论 |
|---|---|---|
| 四服务 FastAPI 项目、注释配置、Dockerfile、单机 Compose | 结构/入口测试和 `docker compose ... config --quiet` | 已完成交付骨架 |
| control 提交/查询、整数状态、Outbox 事务、Redis 注册和可续租容量 | 单元、PostgreSQL/Redis 组件测试 | 组件层已完成，审计持久化仍待接线 |
| PPT 算子共享落盘、原子 manifest、单终态回调、N 路容量 | PPT Conda 单测、启动检查、合成视频烟测 | 算子侧已实现 |
| orchestrator 的 PPT 新适配器、manifest 校验、幂等回调、对账和续租客户端 | `tests/test_ppt_slice_adapter.py` | 组件层已实现，尚未接入常驻节点执行循环 |
...
因此，“四服务已搭建”只表示可独立配置、启动和部署的项目边界已形成，不表示 A 提交后已经能够自动跑完整 DAG。达到生产可用仍需完成第 2-5 阶段及真实 PostgreSQL/Redis/Kafka/算子契约 Harness。
...
3. 先接 PPT_SLICE，验证提交到查询和长期文件闭环。
   - PPT 算子共享落盘、原子 manifest、一次终态回调和容量续约必须先贯通。
4. 再接 OCR 和关键词。
