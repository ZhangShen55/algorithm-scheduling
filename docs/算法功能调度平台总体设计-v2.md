...
Redis 保存心跳 TTL 和原子容量租约。必须先租约成功再调用实例，结束或超时释放。PPT 等异步长任务在“已受理”后继续由 orchestrator 续租，数据库完成事务提交后才释放，避免原 TTL 到期后同一实例被超额分配。
...
完整物理字段、约束和索引由 `algorithm-scheduling-platform/migrations/0001-0003` 定义；
`0004_schema_comments.sql` 使用 PostgreSQL `COMMENT ON TABLE/COLUMN` 为每张表和每个字段补充
中文说明。以后新增字段必须在新的前向迁移中同步增加字段注释，不得只修改已执行过的旧迁移。
...
均保留不动；正式部署应由迁移工具按顺序在目标业务库执行 `0001-0004`，而不是复制测试库。
...
| control 提交/查询、整数状态、Outbox 事务、Redis 注册和可续租容量 | 单元、PostgreSQL/Redis 组件测试 | 已有组件，需在里程碑 1 接入真实常驻服务并形成闭环证据 |
| PostgreSQL 调度表、依赖关系、索引和中文注释 | `0001-0004` 迁移及迁移约束测试 | DDL 基线已形成；本机正式业务库尚未执行迁移 |
...
5. 在正式业务库按顺序执行 `0001-0004`，验证 10 张表及全部中文表/字段注释。
...
5. 使用集成测试专用契约 Stub 验证调用和结果持久化；PPT 真实算子在阶段二接入，不作为里程碑 2 的阻塞条件。
