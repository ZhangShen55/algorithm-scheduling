...
| 图编号 | 形成时间 | 视角 | 状态 |
|---|---|---|---|
| ARCH-001 | 2026-08-07（提交 `3363d3e`） | 平台、基础设施与全部算子的一体化组件全景 | 历史保留，组件关系仍有参考价值 |
| ARCH-002 | 2026-08-07 | 离线、在线和运维三条边界 | 当前有效 |
| SEQ-001 | 2026-08-07 | 方案 C 基础任务闭环时序 | 当前有效，2A 已验证、2B 部署验收中 |
| ARCH-003 | 2026-08-15 | 单机三卡部署、暴露面与镜像 attestation | 当前有效，里程碑 2B 验证视图 |
...
ARCH-003 在不替换 ARCH-001/ARCH-002/SEQ-001 的前提下，补充当前 2B 的物理部署和
验收边界。图中的 `127.0.0.1` 仅表示宿主机暴露面；容器内部仍通过
...
因此，OCR、离线 ASR、文本分析、VBas、Facerec 和 ScreenDet 等同步调用通常不需要额外的逐任务回调。它们的心跳只报告实例级事实，例如 `model_ready`、`inflight` 和容量，不报告课程 DAG 状态。
...
当前八个算子的 `app.main` 都已调用公共 `install_operator_runtime`，具备 `/ops/health`、`/ops/status`、`/ops/drain` 和注册心跳的接入骨架。该功能默认关闭，只有设置 `PLATFORM_REGISTRATION_ENABLED=true` 及 Control 地址、实例 ID、服务 URL 等环境变量后才会真正注册。通用中间件可统计同步 HTTP 请求和 WebSocket 会话；PPT 已使用任务管理器提供真实 `inflight`，OCR 已提供模型就绪判断。其余算子的 `model_ready`、实际并发、内部队列和过载行为仍需逐项目验真，不能把“已安装注册客户端”视为已完成生产接入。
...
里程碑 2B 的权威拓扑为三张 GPU，每张卡部署一套六类 GPU 算子；CPU profile 另部署
三套 PPT 和三套 Text Analysis，共 24 个算子实例：
...
CPU:  ppt_slice × 3、text_analysis × 3，workers=1
```
...
FaceRec 三实例必须同时运行；PPT 三实例和 Text Analysis 三实例必须逐实例 Smoke；最终
24 实例必须同时 ONLINE，再执行 full preflight 和八类 full Smoke。
...
| orchestrator 的 PPT 适配器与执行链路 | 真实回调模型、动态区间持久化、失败终态、符号链接防护、幂等和续租组件测试 | 适配器合同已对齐；factory 尚未注入 handler，完整运行链路未打通 |
| 算子注册客户端 | 独立 wheel 构建、隔离导入并安装到本机算子环境 | Python 3.10+ 轻量交付成立；生产制品库发布流程待建设 |
| 本机全部算子真实推理 | ASR、FaceRec、OCR、ScreenDet、VBas、Text Analysis、PPT 合成视频逐项运行 | 具备里程碑 2 接入前提；真实课程 P 视频仍待补充 |
...
| 八类算子统一镜像 | 历史 SHA 已在 x86_64 服务器完成八镜像构建和 revision inspect | 构建能力已证明；最终发布 SHA 仍须统一重建并 attestation |
| 三卡 24 实例与全部业务验收 | preflight、逐实例端点、GPU/Smoke/报告工具已实现 | 2B-Deploy 进行中；尚未取得 24 同时 ONLINE、18 GPU 活动和全部泳道证据 |

因此，“四服务已搭建”只表示项目边界和部分组件已形成，不表示 A 提交后已经能够自动跑完整 DAG。
当前方案 C 的 control 事实闭环和 orchestrator 契约 Stub 通用运行时已经分别取得证据。
正在执行里程碑 2B 的三卡部署验收：用最终 SHA 重建并证明四个平台镜像和八类算子镜像，
按 gpu0/gpu1/gpu2/cpu profile 验证 24 实例，再执行真实推理、反例、压力和恢复。组件合同、
历史镜像构建或健康检查通过，都不等于 Kafka 驱动的真实业务泳道已经完成。
...
- 请求由 TestClient 构造，不是实际 A 服务发出。
- 注册和心跳由测试客户端构造，不是八个真实算子容器统一上报。
- 没有下载真实视频、执行 FFmpeg 或调用真实算法。
- 没有由 Worker 产生最终节点结果和完整课程终态。

当前 2B 必须观察并留存：最终镜像 revision、四个平台 runtime preflight、四个 profile
和 full operator preflight、18 个 GPU 实例活动/停止/恢复、六个 CPU 实例逐实例 Smoke、
FaceRec 三实例并行就绪、八类 full Smoke、反例、压力和恢复。真实证据齐备前，不得勾选
OpenSpec 7.4/7.5，也不得宣称 PPT、ASR、视觉或在线业务泳道已经贯通。
...
**里程碑 2B：三卡真实算子部署验证（进行中）**

1. 使用同一个最终 `EXPECTED_GIT_SHA` 构建并证明四个平台镜像和八类算子镜像。
2. 在三张 GPU 上各部署六类 GPU 算子，CPU 部署三套 PPT 和三套 Text Analysis；逐
   gpu0/gpu1/gpu2/cpu profile 验证后，再验证 24 实例同时 ONLINE。
3. 对 18 个 GPU 实例逐一执行真实推理、CUDA PID/UUID/cgroup 采样、停止残留验证、重启和
   首次心跳恢复；确认推理进程确实位于指定 GPU。
4. 保持 FaceRec 三实例同时运行，PPT/Text Analysis 六个 CPU 实例逐实例 Smoke，随后执行
   八类 full Smoke；对照现有算子响应，不修改已批准业务字段。
