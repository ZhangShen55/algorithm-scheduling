...
| 八类算子统一镜像 | `7efac20` 已在 x86_64 三卡服务器完成八镜像构建、revision inspect 和真实启动 | 该 release 的镜像与启动能力已证明；任何后续发布 SHA 仍须重新 attestation |
| 三卡 24 实例与全部业务验收 | 24 实例全部 ONLINE，GPU `15/18`，CPU Smoke `6/6`，八类 full Smoke `8/8`，权威报告 332 条 | 2B-Deploy 现场执行已完成但验收失败；FaceRec GPU 证据失败，243 条反例/压力用例未执行，完整业务泳道未贯通 |

因此，“四服务已搭建”只表示项目边界和部分组件已形成，不表示 A 提交后已经能够自动跑完整 DAG。
当前方案 C 的 control 事实闭环和 orchestrator 契约 Stub 通用运行时已经分别取得证据。
里程碑 2B 已对 `7efac20cf980ee64ea78fe297af6dfdfb2df5b28` 完成一轮三卡现场执行。
四个平台服务与四类基础设施保持 healthy，24 个算子实例全部注册且 ONLINE，
八类 full Smoke 全部通过。但 FaceRec 三实例的 GPU Harness 探针失败，且 217 条反例和
26 条压力用例未执行，权威报告因此为“失败”。这些直接 Smoke 也不等于 Kafka 驱动的
真实业务泳道已经完成。
...
- 2A 的运行时注册和心跳由契约 Stub 产生；2B 虽已由 24 个真实算子上报，
...
OpenSpec 7.4/7.5 现可依据镜像内注册客户端、实际 24 实例注册和部署预检证据标记完成；
...
2026-08-17 首轮现场结果为：24 实例注册与八类 full Smoke 达标，GPU 证据 `15/18`；
