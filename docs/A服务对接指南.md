...
| `50301` | 在配置的累计等待时间内始终没有在线容量 | 记录追踪标识；确认是否需要稍后重新发起 |
| `50302` | Control 容量服务在允许恢复时间内持续不可用 | 平台依赖故障；有界退避并告警 |
| `50201` | VBas 建连、可恢复 HTTP 或响应协议错误重试耗尽 | 保留请求标识和追踪标识，按业务策略有限重试 |
| `50401` | VBas 响应超时重试耗尽或在线总预算耗尽 | 不要立即形成重试风暴；核对算子负载后有限重试 |
| `50000` | 确定性平台错误或未分类内部错误 | 记录追踪标识并告警，不按普通容量不足处理 |

这些业务码不表示 Control Service 已创建离线任务或发布 Kafka。VBas 在线 JSON 请求从进入网关
开始使用单一 600 秒平台预算；容量等待上限默认 300 秒，并与 Control 恢复、VBas 调用和有限重试
共享总截止时间。A 服务及其反向代理建议配置 630 至 660 秒超时；只增大 A 的超时不能替代平台内部
恢复，A 主动取消请求会使 Gateway 取消后续工作并释放租约。
...
| HTTP 200、外层 code=50301 | 是，有界 | 网关已等待至 `acquire_wait_timeout_seconds` 仍未取得容量；A 记录 trace 后按业务策略重试并告警 |
...
租约申请的重试间隔默认从 `0.2` 秒开始，按 `0.4 -> 0.8 -> 1.6 -> 2.0` 秒退避，并带少量
随机抖动；该等待发生在网关请求内部。所有重试都应设置最大次数、总时间预算和抖动，不要并发重发同一大图。平台在线接口没有请求级
...
7. 人为耗尽容量，确认请求先在 Gateway 等待，只有等待上限耗尽才返回 50301，且不会创建离线任务。
8. 分别制造 Control 持续不可用、VBas 建连/协议失败和读取超时，确认最终得到 50302、50201、50401。
9. 制造可恢复 VBas 失败，确认最多三次调用、实例重选、请求身份不变且最终租约归零。
10. 在容量等待和 VBas 调用阶段主动取消，确认没有活动租约或后台调用残留。
