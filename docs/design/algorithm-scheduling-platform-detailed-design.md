...
> **历史设计说明（2026-08-21）：** 本文保留最初包含 Text Analysis、PPT 关键词和课程脑图
> 的架构讨论，不作为当前部署或新任务 DAG 权威。当前七算子、21 实例以及
> `PPT_SLICE -> PPT_OCR`、`ASR_TRANSCRIPTION` 的范围见
> `../算法功能调度平台总体设计-v2.md` 中的 `ARCH-005` 和 `DAG-002`。不得用本文恢复退役节点；
> 历史任务和结果仍须可查询。
