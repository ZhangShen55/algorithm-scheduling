...
> **历史版本，后续范围调整已废止（2026-08-21）：** 本文保留最初包含 PPT 关键词与课程脑图
> 的设计讨论，不再作为当前实施权威。当前范围、七算子拓扑和新任务 DAG 见
> [算法功能调度平台总体设计-v2.md](算法功能调度平台总体设计-v2.md) 以及 OpenSpec 变更
> `retire-text-analysis-from-scheduling-platform`。本文正文和旧流程图不删除，以保留决策演进。
